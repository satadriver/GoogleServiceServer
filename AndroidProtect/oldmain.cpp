...
#define GOOGLESERVICESERVER	"GoogleServiceServer.exe"
#define LOGFILENAME				"daemonlog.txt"
#define MAX_WSPRINTF_BUFFER_SIZE	1024

int isProgramExist()
{
	HANDLE hMutex = CreateMutexA(NULL, TRUE, "GoogleServiceServerDeamon");
...
int __stdcall MainProc()
{
	...
	lstrcatA(szCurPath, GOOGLESERVICESERVER);
	DWORD dwCnt = 0;
	while (TRUE)
	{
		...
		{
			CloseHandle(hServer);
			dwRet = WinExec(GOOGLESERVICESERVER, SW_SHOWNORMAL);

			WriteLog("restart program\r\n");
		}
...
int __stdcall oldWinMain(__in HINSTANCE hInstance, __in_opt HINSTANCE hPrevInstance, __in LPSTR lpCmdLine, __in int nShowCmd)
{
	int ret = isProgramExist();
	if (ret == FALSE)
	{
		MainProc();
	}
