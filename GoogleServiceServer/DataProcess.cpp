...
DWORD __stdcall DataProcess::DataProcessMain(LPDATAPROCESS_PARAM lpparam, char** lpdata) {
	__try {
		...
		if (lphdr->reservedname & PACKET_COMPRESS_FLAG)
		{
			...
			if (payloadsize <= 2 && *(USHORT*)payload == 0x9c78)
			{
				delete[]unzipbuf;
				return 0;
			}
			...
			if (iret != Z_OK) {
				delete[]unzipbuf;
				wsprintfA(szout, "uncompress error,compressed data size:%u ,uncompressed data size:%u,src data size:%u,user:%s,imei:%s\r\n",
					payloadsize, unzipbufLen, uncompsize, username, imei);
				WriteLogFile(szout);
				return -5;
			}
